...
static int quotafs_getattr(const char *path, struct stat *stbuf)
{
	int res;
...
static int quotafs_access(const char *path, int mask)
{
	int res;
	char real_path[MAX_PATH];
	sprintf(real_path, "%s/%s", src, path);
...
static int quotafs_readdir(const char *path, void *buf, fuse_fill_dir_t filler,
...
static int quotafs_mknod(const char *path, mode_t mode, dev_t rdev)
{
	int res;
	char real_path[MAX_PATH];
	sprintf(real_path, "%s/%s", src, path);
...
static int quotafs_open(const char *path, struct fuse_file_info *fi)
{
	int res;
	char real_path[MAX_PATH];
	sprintf(real_path, "%s/%s", src, path);
...
static int quotafs_read(const char *path, char *buf, size_t size, off_t offset,
...
static int quotafs_write(const char *path, const char *buf, size_t size,
		off_t offset, struct fuse_file_info *fi)
{
	int res;
...
static int quotafs_mkdir(const char *path, mode_t mode)
{
	int res;
	char real_path[MAX_PATH];
	if (strcmp(path, path_cur_size) == 0 || strcmp(path, path_max_size) == 0)
...
static int quotafs_truncate(const char *path, off_t size)
{
	int res;
	char real_path[MAX_PATH];
	if (strcmp(path, path_cur_size) == 0 || strcmp(path, path_max_size) == 0)
...
static int quotafs_chmod(const char *path, mode_t mode)
{
	int res;
	char real_path[MAX_PATH];
	if (strcmp(path, path_cur_size) == 0 || strcmp(path, path_max_size) == 0)
...
static int quotafs_chown(const char *path, uid_t uid, gid_t gid)
{
	int res;
	char real_path[MAX_PATH];
	if (strcmp(path, path_cur_size) == 0 || strcmp(path, path_max_size) == 0)
...
static int quotafs_rmdir(const char *path)
{
	int res;
	char real_path[MAX_PATH];
...
static int quotafs_rename(const char *from, const char *to)
{
	int res;
...
static int quotafs_unlink(const char *path)
{
	int res;
	char real_path[MAX_PATH];
...
static int quotafs_release(const char *path, struct fuse_file_info *fi)
{
    int res = close(fi->fh);
    if (res == -1)
		return (-errno);
...
static struct fuse_operations quotafs_oper = {
	.getattr	= quotafs_getattr,
	.access		= quotafs_access,
	.readdir	= quotafs_readdir,
	.mknod		= quotafs_mknod,
	.open		= quotafs_open,
	.read		= quotafs_read,
	.write		= quotafs_write,
	.mkdir		= quotafs_mkdir,
	.chmod		= quotafs_chmod,
	.truncate	= quotafs_truncate,
	.chown		= quotafs_chown,
	.rename		= quotafs_rename,
	.rmdir		= quotafs_rmdir,
	.unlink		= quotafs_unlink,
	.release	= quotafs_release,
};
